...
| 구현 상태   | 핵심 구현 완료. 자동 검증 통과, 외부 운영 검증과 출시 승인 대기      |
...
- Firebase Admin 서비스 계정 파일과 외부 번역 키 파일이 Git에 추적되어 있었다.
- 레거시 서버 코드에 Naver 번역 자격증명 문자열도 포함되어 있었다.
- 루트 `node_modules` 파일 88개가 Git에 추적되어 있다.
- 현재 트리에서는 자격증명 파일과 하드코딩 문자열을 제거했지만, 공급자 폐기와 Git 이력 재작성 전에는 과거 커밋의 노출이 해결되지 않는다.
