...
1. Firebase Admin/Google Cloud 서비스 계정과 번역 API 자격증명을 목록화한다.
2. Google Cloud/Firebase 콘솔에서 해당 key를 폐기한다.
3. 기존 key로 인증이 실패하는지 별도 안전한 환경에서 확인한다.
...
5. 실제 secret 문자열이 다른 파일에 복제됐는지 검사하고, 발견 시 generated replacement expressions 파일로 추가 정리한다. secret 원문을 shell history나 문서에 입력하지 않는다.
