...
위 자동 검증 결과는 2026-08-26 Node `24.19.0` 환경에서 기록했으며, exam-coach W1~C2가 포함된 PR #33의 최종 `secret-scan`/`verify`도 2026-09-04 통과했다. 이후 `origin/master`의 추가 변경(#35)은 다음 릴리스 전 전체 게이트를 다시 실행해 증거를 갱신한다. 외부 서비스와 실제 운영 데이터가 필요한 아래 항목은 별도 증거가 생기기 전까지 완료로 표시하지 않는다.
