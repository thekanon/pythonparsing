...
export default async function ExamCoachLearningPage({
  ...
  const selected = selectReviewedUnit(reviewedContent, unit);
    ...
    <div className="page-shell">
      ...
      >
        <Link
          ...
          className={unit === "sql" ? "button button-primary" : "button button-secondary"}
          aria-current={unit === "sql" ? "page" : undefined}
        >
          SQL 학습 단위
        </Link>
        <Link
          href="/exam-coach/learn?unit=c"
          className={unit === "c" ? "button button-primary" : "button button-secondary"}
          aria-current={unit === "c" ? "page" : undefined}
        >
          C 학습 단위
        </Link>
